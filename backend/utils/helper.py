...
def classify_emotion(emotion):
    positive = ['happy', 'calm', 'relaxed', 'neutral']
    negative = ['stressed', 'angry', 'sad', 'fearful', 'disgusted']
    if not emotion: return "Unknown"
    e = emotion.lower()
    if e in positive: return "Positive"
    if e in negative: return "Negative"
    return "Neutral"

def classify_posture(posture):
    if not posture: return "Unknown"
    p = posture.lower()
    if p in ['upright', 'straight', 'good']: return "Good"
    if p in ['slouched', 'forward', 'bent']: return "Poor"
    return "Neutral"
...
def calculate_health_score(data):
    ...
    if data.get('smoking')        == 1: score -= 10
    if data.get('diabetes')       == 1: score -= 10
    if data.get('family_history') == 1: score -= 5

    # Presage stress bonus/penalty (display-only but affects score slightly)
    stress = data.get('stress_score')
    if stress is not None:
        if stress >= 80: score -= 10
        elif stress >= 60: score -= 5
